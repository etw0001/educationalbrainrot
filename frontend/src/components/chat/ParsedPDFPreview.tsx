...
export function ParsedPDFPreview({ data, className }: ParsedPDFPreviewProps) {
  ...
  return (
    <div
      className={cn(
        "border-b border-border bg-muted/30 shrink-0 max-h-[min(40vh,28rem)] overflow-y-auto scrollbar-thin",
        className
      )}
    >
      <div className="max-w-3xl mx-auto px-4 py-3 space-y-3 text-xs">
        ...
        <div className="space-y-4 pt-1">
          {pageList.map((p) => {
            ...
            return (
              ...
              >
                <p className="font-semibold text-sm">Page {p.page}</p>
                ...
                {tables.length > 0 && (
                  <div className="space-y-1">
                    ...
                    {tables.map((t, i) => (
                      <div
                        key={i}
                        className="overflow-x-auto rounded border border-border/60 bg-muted/40 p-2"
                      >
                            ...
                            {(Array.isArray(t.rows) ? t.rows : []).map((row, ri) => (
                              <tr key={ri}>
                                {(Array.isArray(row) ? row : []).map((cell, ci) => (
                                  <td
                                    key={ci}
                                    className="border border-border/50 px-2 py-0.5 align-top"
                                  >
                                    {cell}
                                  </td>
                                ))}
                        ...
                        <div key={i} className="space-y-0.5">
                          {fig.data_base64 ? (
                            <img
                              ...
                              className="max-h-32 max-w-full rounded border border-border object-contain bg-background"
                            />
                          ) : (
